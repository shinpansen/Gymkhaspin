...
func _play_engine_sound(delta) -> void:
	...
	# Tricky: Lower regording is not 500 but acutally 700 rpm. Adjusting pitch accordingly
	if lower_rpm == 500.0: 
		lower_rpm = 700.0
	var step: float = 700.0 if lower_rpm == 700.0 else 500.0
	var rpm_ratio: float = clamp((rpm - lower_rpm) / step, 0.0, 1.0)
	...
	# Pitch and volume
	lower_stream_player.volume_db = linear_to_db(clamp((1.0 - rpm_ratio), 0.01, 1.0))
	lower_stream_player.pitch_scale = rpm / lower_rpm
	if upper_stream_player != lower_stream_player:
		upper_stream_player.volume_db = linear_to_db(clamp(rpm_ratio, 0.01, 1.0))
